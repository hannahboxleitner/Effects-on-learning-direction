...
Most of the data that we will be looking at for descriptive statistics are things we are already familiar with such as mean, median, maximum and minimum, as well as the standard deviation. But the authors of the paper also had a look at skewness and kurtosis. 

We will start with Skewness. To keep it simple, (EXPLAIN SKEWNESS)

# Descriptive Statistics (Table 3)

We want to reproduce Table 3 in Terai et al. (2021) which shows us the descriptive statistics of the target items of the study. Unfortunately the code for this table was not included in the R code published by the authors. Although, that is not an issue as we have the data in the form of a csv file! This is great for us. We have the organized data ready for us to analyze. Thus, we will learn how to reproduce the data of Table 3. 

In order to compute the mean, standard deviation, range, skew, kurtosis, and n for each variable we need these libraries: dplyr, psych, tibble, knitr, kableExtra (I think?).

NUMBERS HERE ARE WRONG FOR SKEW AND KURTOSIS AND I CANNOT FIGURE OUT WHY OMG

```{r}

##The packages we need include:
library(dplyr)
library(psych)
library(knitr)
library(kableExtra)
...
##For the next step we have to load our data, since the data was already read with the read.csv command we simply need to fix any stray spaces like "berth " 
...
items <- dat %>%
  ...
  mutate(
    L2Frequency_log = log10(L2Frequency + 1),
    L1Frequency_log = log10(L1Frequency + 1)
  )

summ <-  function(x, pearson = FALSE){
  d <- psych::describe(x)
  kurt <- if(pearson) d$kurtosis + 3 else d$kurtosis
  data.frame(
    Mean     = round(d$mean, 2),
    SD       = round(d$sd, 2),
    Median   = round(d$median, 2),
    Range    = paste0(d$min, " - ", d$max),
    Skew     = round(d$skew, 2),
...
##Now we build the two subtables

tab_L2 <- rbind(
  cbind(Variable = "Frequency (L2, English, log10)", summ(items$L2Frequency, pearson = TRUE)),
  cbind(Variable = "Syllables (L2)",           summ(items$Syllables, pearson = TRUE)),
  cbind(Variable = "Letters (L2, Alphabet)",  summ(items$Alphabet, pearson = TRUE))
)


tab_L1 <- rbind(
  cbind(Variable = "Frequency (L1, Japanese)",   summ(items$L1Frequency, pearson = TRUE)),
  cbind(Variable = "Letters (L1, WordLength)",    summ(items$WordLength, pearson = TRUE)),
  cbind(Variable = "Mora (L1)",                  summ(items$mora, pearson = TRUE)),
  cbind(Variable = "Familiarity (participants)", summ(items$familiarity, pearson = TRUE))
)

##Now we can print it prettily (or we could do Vew(tab_L2); View(tab_L1))
...
# (1|ID) + (1|ItemID): random intercepts for participants anditems (to account for repeated measures)
