...
inside a project. We also want to declare the learning direction (Condition) as a factor.
Here we need to tell R that our variables represent categories and not
numbers.  Converting our variables into factors now will also be useful for any
other statistical analysis such as an ANOVA which we will be doing in
later steps.
...
```{r}
library(tidyverse) #double: loaded before
dat<-read.csv(file = here("data", "dataset1_ssla_20210313.csv"), header = TRUE)
dat$ItemID <- trimws(dat$ItemID)
...
# load dplyr library
library(dplyr) # double: loaded before
...
# knitr and kableExtra packages have to be loaded for this to work.

library(knitr) # double: loaded before
library(kableExtra) # double: loaded before
...
We have created a table that summarizes the descriptive statistics of the
two types of post-tests, where the measures of scores in the two
...
longitudinal data that is not continuous (Gurka & Edwards 2011).
...
Accuracy (Answer) from Test, Condition, and their interaction with the glmer() function. We want
...
tests. Before we work through and understand the code step by step like the
...
proficiency by itself did not strongly predict L1 Production accuracy of the participants.
...
Vocabulary Size or learning condition by themselves, so the interaction is what matters here.
...
outcome is statistically significant with a p-value of .009,
indicating and confirming that the model with Condition and
...
It seems that vocabulary learning is not something that can be generalized. While retrieval practice is quite effective, the direction
