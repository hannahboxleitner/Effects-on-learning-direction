...
```{r}
# Extract data from model frame
model_data <- model.frame(fit1.1)

# Build a dataframe with residuals and fitted values on response scale
resid_df <- data.frame(
  Test = model_data$Test,
  Condition = model_data$Condition,
  Fitted = fitted(fit1.1, type = "response"),
  Residual = residuals(fit1.1, type = "response")
)

# Compute observed values
resid_df$Observed <- resid_df$Fitted + resid_df$Residual
```
...
ggplot(emm_df, aes(x = Test, y = prob, group = Condition, color = Condition, linetype = Condition)) +
  ...
  # Residuals (raw data points)
  geom_jitter(data = resid_df, aes(x = Test, y = Observed, color = Condition),
              width = 0.1, alpha = 0.4, size = 2, inherit.aes = FALSE) +
...
ggplot(emm_df, aes(x = Test, y = prob, group = Condition, color = Condition, linetype = Condition)) +
  ...
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1) +
  geom_jitter(data = resid_df, aes(x = Test, y = Observed, color = Condition),
              width = 0.1, alpha = 0.4, size = 1, inherit.aes = FALSE) +
  scale_color_paletteer_d("beyonce::X11") +
  scale_linetype_manual(values = c("solid", "dashed")) +
  scale_y_continuous(name = "Accuracy Rate", limits = c(0.0, 0.75)) +
  xlab("Test Type") +
...
```{r}
# Visualize effect of Test, conditioned on Condition
visreg(fit1.1, "Test", by = "Condition", scale = "response", 
       overlay = TRUE, points.par = list(col = "grey70", pch = 19))
