...
in memory (Terai et al. [-@2021]: 1116-1117). Previous findings
...
& Hastie [-@1992]). While it may look confusing and maybe intimidating,
it is actually quite the important step. Basically, we want to compare
our two groups (L1 -\> L2 vs. L2 -\> L1). We use 'simple coding' where
our first Condition (L1 -\> L2) gets coded as -0.5 and our second
Condition (L2 -\> L1) as +0.5. This makes interpretations easier when we
have interactions since the intercept represents the overall mean across
...
interpreting the outcomes are not always straightforward. But usually there is always a solution. The insights we can gain from a
nicely fitted model make all this effort worthwhile.

Due to the extensive research by Terai, Yamashita and Pasich [-@2021] who published their data set and R code along with the paper, we were able to reproduce their findings and gain hands-on experience with modeling. This not only helps with our
