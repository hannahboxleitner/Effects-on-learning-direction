...
Japanese (their L1) and English (their L2). Terai et al. \[-\@2021:
1121-1122\] formulated two hypotheses:
...
inside a project. We also want to declare the test type (`Test`) and the
learning direction (`Condition`) as a factor variables as these
variables represent categories rather than numerical values. Converting
our variables into factors now will also be useful for the statistical
analysis such as an ANOVA which we will be conducting in future steps.
...
We also apply **contrast coding**, also called **treatment coding**
(Chambers & Hastie [-@1992]). While it may look confusing and maybe
intimidating, it is actually quite an important step. Basically, we want
to compare our two groups (L1 -\> L2 vs. L2 -\> L1). We use **simple
coding** where our first Condition (L1 -\> L2) gets coded as -0.5 and
our second Condition (L2 -\> L1) as +0.5. This makes interpretations
easier when we have interactions since the intercept represents the
...
reproduce these findings cannot be found. It is neither part of the
dataset accessible on IRIS, nor does it come up anywhere in the authors'
R code. Therefore, we will only focus on the descriptive statistics of
...
in Table 3 in Terai et al. [-@2021]: 1123.
...
of the study. Unfortunately, the code for all the tables was not
included in the R code published by the authors. But since we have the
...
We have already loaded the data that we will need for our table, but
...
To investigate accuracy, we first need to load our dataset and the dplyr
library (in accordance with previous data imports, we are using the here
package in this case as well). We want to process the collected results
for each participant — their ID, how many items they answered, and how
many they got right — and visualize them in a boxplot.

```{r}
#| warning: false

# load dataset as dat.acc using the here package
