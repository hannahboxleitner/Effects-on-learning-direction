...
through the entire data analysis pipeline, from importing the data to running logistic regression models and visualizing
results. By the end of this chapter, it should become clear how
...
used as stimuli and responses are given in L1, vs. L1 to L2, where it is the other way around. Opposed to
...
Our aim in this chapter is not only to replicate the study by Terai et al. (2021), but also to learn how to apply mixed-effect modeling in R to research questions in second language acquisition. 
...
participant and item, including Answer (whether the participant gave the correct answer (1 = correct, 0 = incorrect)), Test (distinguishing two test types: L1 Production or L2 Production), Condition (learning direction L2 -> L1 or L1 -> L2), and so on. But more on that later.
...
Skewness and Kurtosis are both measures of shapes and distribution of a data set in qualitative methods. Skewness measures the asymmetry of distribution. Simply put, it describes the amount of which the distribution differs from a completely symmetric shape. 
...
The raw data set has multiple rows per word because each participant contributed responses. But for descriptive statistics of the items, we only want one row per word. So we take the first value for fixed properties, such as frequency or length, which are always the same and then compute the mean familiarity across the participants.
...
Perfect, the next and easiest part of building up our two subtables is captioning them and then finally printing them. Here we need the kable() and the kable_classic() function.
...
Now, though we want pretty histograms so we will add a tiny helper for style. It will also keep our histograms consistent:
...
We will start with a histogram of L2 Frequencies. To do this we use the ggplot() command. Our first line of code here 'ggplot(data = items, aes(x = L2Frequency))' tells ggplot to basically use the items table and put the L2 frequencies of each word onto the x-axis. 'geom_histogram(bins = 15) 
...
Just to be sure we understand how to build and understand these histograms, we want to create two more. For L2 letters and L1 letters (Japanese orthographic length).
```{r}
...
The *Random effects* section puts out the estimated variability in the intercept across subjects (ID) and items (ItemID), accounting for differences in overall accuracy between participants and between items.
...
Next we will standardize the proficiency, which means we transform Vocab scores into z-scores which will also help us with the interpretation.
...
It seems that vocabulary learning is not something that can be generalized. While retrieval practice is robustly effective, the direction of practice depends on who the learner is. Due to the extensive research by Terai, Yamashita and Pasich (2021) who published their data set and code along with the paper, we were able to reproduce their findings and gain hands-on experience with modeling. This not only helps with our understanding of the research, but it also sharpens our skills as analysts of second-language data. 
