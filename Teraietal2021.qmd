...
In a later step the command `trimws()` will be used and removes space from the beginning and end of the text which helps to prevent issues like "berth " vs. "berth" being treated as different
...
R code. Although, it is common that participant data is not available to protect their identities. Therefore, we will only focus on reproducing the descriptive statistics of
...
small machine that we will name `quick_stats()`, it will do the calculations for
us. `quick_stats \<- funcion(x)` creates this function with the input 'x' which
...
summary of the output of `quick_stats()`. With our helper function `desc()` we
...
statistics. In the next step, we will round the numbers to two decimals to make the table actually readable.
...
caption with the command `caption = "..."`. With the command `align = "lrrrrrr"` we control the way the text is aligned in each column. the 'l'
...
`ggplot()` command. The first line of code here `ggplot(data = items,
...
where the numbers in the table come from, we add reference lines:
...
Now we want to do the same with the results of reliability testing, namely
the alpha coefficients. To display these results, we want to create a table similar to Table 5 in the paper [@2021: 1126]. 
To accomplish this, we first need to apply the `alpha()` function to calculate Cronbach’s alpha and 95% confidence intervals for two vocabulary tests (A and B) at two proficiency levels (L1 and L2).
...
Then we use sprintf() to create a summary table showing the reliability estimates and CIs for each test and level, and format strings for the table.
...
Lastly, we want to display the results in a clean, formatted table, for which we use the `kable()` function.
...
paper [@2021: 1126], we notice a difference in
...
@2021 used generalized linear mixed-effect models (GLMMs)
...
Size (L2 Proficiency), as well as interaction between two variables. Three models were chosen for this analysis, the first one to
analyze the relationship between learning conditions and production
tests (RQ1) [@2021: 1125]. The second and third model
...
As we recall from the introduction, the first research question of @2021's study was: Which learning direction is more effective
...
@2021 fit a logistic mixed-effects regression model predicting
...
The following code chunk shows how Terai et al. fit the model without interaction `fit1`. Subsequently, the authors calculated AIC (Akike Information Criterion) for their model. We will explain more about this criterion later.
...
formula, both fixed effects (Test answers and learning condition) and random
...
inter-item variability. By specifying `family = binomial`, we can tell the
`glmer()` function that the outcome variable is binary (correct answer =
1, incorrect answer = 0). The last argument specifies an optimizing
algorithm, which helps the model to converge. Terai et al. used the `summary()`
function to look at the output of this model.

Without interaction, the model would assume that the effects of Test and
Condition are additive and do not depend on each other. The AIC is a measure used to estimate the
...
fitting the model with interaction to compare the two.
...
Next, the authors fit a model similar to the first one, but with the difference
...
For this model `fit1.1`, the `glmer()` function was used as well, but with a change in the formula (connect the effects with * instead of +). Aditionally to the model and the AIC, @2021 also calculated confidence intervals for the model using the function `confint()`.
...
Now after calculating AICs for both versions of the model, it becomes apparent
...
**Test Type** (Estimate = -0.976, SE = 0.105, z = -9.315, p \< .001) [@2021: 1126]. We
...
Moving on to **Learning Condition**, the authors found no
statistically significant main effect: Condition2: Estimate = -0.038, SE
= 0.103, z = -0.366, p = .714) [@2021: 1126]. In this case, the **p**-value is \> 0.05,
...
much when moving from one learning condition to the other. 
However, the
**interaction** of Test Type and Learning Condition (see row
"Test2:Condition2") was shown to be significant (Estimate = -0.446, SE = 0.206, z =
-2.169, p = .030) [@2021: 1126]. The interaction term demonstrates how the effect of Test
...
@2021 conducted further analysis and comparisons of the two models, using the command
...
effects analysis.
First, the authors conducted a comparison of the model with and without interaction using the `anova()` function.
...
Since a significant effect of the interaction was found, @2021 compared the
two models with the `anova()` function to test if the interaction improves
the model. It produces a **p**-value of 0.03071. Being smaller than 0.05,
...
`fit1` (without interaction). 

Next, @2021 checked simple main effects using the `testInteractions()` function in both directions.
...
With the `testInteractions()` function, it is
...
with this function, the interaction can be further unpacked, to find
...
important information for the interpretation of this model as reported in @2021. We want to go through the authors' statements and connect them
...
learning:** p \< .001, d = 0.75, \[0.47, 1.04\]) [@2021: 1126]. Now, we want
...
learning conditions. A small **p**-value tells us that an effect is unlikely due to chance, but does not say much about how big or important it is. This is
where effect size comes into play: The function `eff_size()` computes the
...
The authors also mention that results showed no main effects of **Learning
Condition** (L1 production test: p = .188, d = -0.19, 95% CI \[-0.46,
0.09\]; L2 production test: p = .082, d = 0.26, \[-0.03, 0.55\]) [@2021: 1126]. This
...
shows **p**-values, which lie above 0.05 in both production tests and
...
Production, and a small positive effect for L2 Production. The confidence intervals in both cases
...
visualizes all important information regarding research question 1 [@2021: 1127].
In the following code chunk, you can see how they set graphical parameters and then created a base `R` plot.
...
This plot shows the interaction between
...
L2 production tests. The x-axis represents Test Type. 
We can see that
...
# To create a ggplot, we need to transform the EMMs back to the original scale of the outcome variable with specifiying the type, and then convert our emmeans object into a data frame:
...
@2021 hypothesized that the effectiveness of learning
direction or word pairs might be dependent on the learners' proficiency
...
variable which, in case of @2021's study, is the accuracy on production tests and
...
proficiency in this study). The random effects are Participant ID and
Item ID so that the model can account for individual differences.

### L1 Production test

We begin by focusing only on the L1 production data. @2021 loaded the data again for this part and saved it under another variable. As we already mentioned before, this is not good practice, so we will continue to use `dat` instead to reproduce the code. @2021 used the command
`filter()`, which only keeps the rows for the L1 Production test. This data is filtered because, while each participant took both L1 and L2 production
tests, the authors of this study found different patterns for both test
types, which is why they chose to analyze them separately
...
includes both main effects and their interaction. It was tested whether the
effect of vocabulary differs between the learning conditions. As @2021 stated in their paper, the interaction of Learning Condition and Vocabulary
...
To formally check which model fits better, @2021 used the command `anova()`
just as they did for RQ1.
...
complex models like this one. @2021 visualized the model `fit2.1` using a base `R` plot.
...
The plot shows a solid line representing the L1 -\> L2 learning condition,
and a dotted line representing the L2 -\> L1 learning condition. The
y-axis shows L1 production accuracy rates of the test, and the x-axis
the scaled scores of the vocabulary size tests [@2021:
1128]. Looking at the plot you can see a nearly flat line for L2 -\> L1 and
...
@2021 used the `testInteractions()` function to test whether the slope of proficiency differs significantly
between conditions as predicted. 
...
significant, Terai et al. examined the simple effects of "s.vocab" across
...
predicting accuracy for when the participants had to produce English words
...
```{r}
# Contrasting our data once again
...
Similar to @2021's first research question, they calculated AICs for both
...
output of the `summary()` function, we see that the Vocabulary Size estimate is
-0.227 (SE = 0.193, z = 1.179, p = .238). Our **p**-value is greater than
0.05, which means the effect is not statistically significant and
proficiency alone can not predict accuracy on the L2 production test.
...
learning directions. In a final step, the interaction term
"s.vocab \* Condition" was considered, which was also not statistically significant
(Estimate = -0.278, SE = 0.154, z = -1.810, **p** = .070).

### Model Comparison

To formally check whether the model with the interaction is a better fit
than the simpler model without the interaction, @2021 compared them using the `anova()` function:

```{r}
anova(fit3, fit3.1)
```

This function was again used to decide if maybe the
...
direction. This can be seen better in a plot which will be printed in the
next step.
...
Just as the numbers of the tests above suggested, this plot shows that while
...
Furthermore, the authors explained that L2
-\> L1 learning is much less influenced by L2 proficiency [@2021: 1128], which we
can see clearly in the graphs by the nearly flat dashed line.
...
rather dependent on learner proficiency.
However, it might be worth highlighting that only a single language pair was tested in this study. Assuming that the above-mentioned results are generalisable L1-L2 effects may be a bit far-fetched.

## Model Diagnostics

Now, before interpreting
these results too confidently, one should also check whether the
...
@2021 used a helper function `vif.mer()` to calculate VIFs for mixed models.
...
participants and items such as we had in this study by @2021. It is completely normal that at times GLMMs can feel
...
Due to the extensive research by @2021,
...
modeling. This chapter does not include the reproduction and explanation of every part of code the authors published, since we focused on what was also discussed in the paper by @2021. In the reproduced code, we only changed minor things, like not reloading the same dataset several times under a different name. This sort of project not only helps with our understanding of the research,
